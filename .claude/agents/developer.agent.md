...
- JPA: `CrudRepository` / `JpaRepository` patterns in `com.umdc.mercury.jpa.sql.repository`
- MongoDB: `MongoRepository` in `com.umdc.mercury.jpa.nosql.repository`
...
- All DTOs are Java records in `com.umdc.mercury.api.v1.to` with Bean Validation annotations (`@NotNull`, `@NotBlank`, `@Valid`)
- Scheduler fixed-rates use `${umdc.scheduler.*}` property placeholders, not hardcoded values
...
- Fully compilable Java source files at correct `com.umdc.mercury.*` package paths
